...
void MINIGAME_pressRightNumber() /* Minigame padrão */
{
	...
	if(minigameMax<9) 
	{
		minigameMax++; /* A sequencia engloba mais numeros */
	}
	c = randomNumber(0,minigameMax); /* Sorteia qual numero deve ser apertado */
	for(i=0;i<MAX;i++) {
		n = randomNumber(0,minigameMax); /* Sorteia os numeros da sequencia */
		sequence[i] = n; 
